...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/03/30 15:46:22 by ebichan           #+#    #+#             */
/*   Updated: 2026/04/13 01:57:36 by ebichan          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void my_pixel_put(t_minirt *minirt, int x, int y, int color)
{
    ...
    dst = minirt->addr + (y * minirt->size_line + x * (minirt->bits_per_pixel / 8));
...
void drawing_picture(t_minirt *minirt)
{
    ...
    while(y < WIN_HEIGHT)
    {
        ...
        while(x < WIN_WIDTH)
        {
            ...
            if(hit.hit)
                my_pixel_put(minirt, x, y, color_to_int(calc_lighting(minirt, hit)));
            else
